...
```jsonc
{
  ...
  "homepage": "https://github.com/you/your-game", // optional; absolute https:// link behind the version badge/chip (repo → its releases page)
...
| `homepage` | — | Link to your game's own page or repository. Must be an absolute `https://` URL — anything else is dropped with a warning and the version tag stays plain text. A GitHub repository URL resolves to its releases page (`…/releases`) as the home-card version chip's and in-game version badge's link (opens in a new tab); any other URL is linked as-is. Also shown in the marketplace listing. `knockbox pack` rejects a non-https value so you fix it here. |
