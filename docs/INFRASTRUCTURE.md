...
| `MarketplacePollMinutes` | `360` | How often registered catalogs are checked for updates to games **enrolled** in automatic updates. `0` disables it. Nothing is enrolled by default, and a pass with an empty enrolment makes no request at all. First tick is jittered so a fleet restarted together doesn't thunder. |
