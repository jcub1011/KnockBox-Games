...
| `BlobTotalQuotaBytes` | `21474836480` (20 GiB) | **The aggregate cap, and the one that actually bounds disk use** — without it the per-lobby figure is only that times the number of lobbies. Size it to the volume `BlobsRoot` sits on: by default that is the container's own writable layer, so on a host with a size-capped overlay filesystem either lower this or point `BlobsRoot` at a mount with room. `0` = no limit, which on a default container layout is a disk-fill vector rather than a convenience. |
| `BlobGraceMinutes` | `5` | How long freshly uploaded bytes are protected from the sweep before any handle references them. Covers exactly one gap — the round trip between a client uploading and registering — and nothing after it: once a handle has existed, releasing it deletes immediately. `0` = no grace, which makes an upload racing a sweep possible. |
| `BlobSweepSeconds` | `300` | Backstop sweep cadence. Startup-only, because a cadence stays fixed for the process while its window (`BlobGraceMinutes`) is read live. Not the reclamation mechanism — the reference count already deletes on the last release — so this only retries a delete that lost to an open read handle and collects the entry an abandoned upload leaves behind. `0` = no backstop. |
| `BlobMaxUploadsPerLobby` | `4` | Uploads one lobby may have streaming at once. Bounds the one hole the grace window opens: a client can start an upload, send nothing, and leave a grace-protected entry behind. `0` = unlimited. |
...
| `AdminSettingsPath` | `admin-settings.json` next to `AdminPasswordPath` | Persisted operator policy: per-game availability (`available`/`disabled`/`staged`), maintenance mode, runtime limit overrides, the room-code blocklist, the live player announcement and the webhook endpoints. Operator policy the server keeps across a restart, because re-applying policy by hand after every deploy is how a platform ships a game it meant to keep hidden. Same requirements as the password file — writable, and on a persisted volume in a container. Unreadable ⇒ platform defaults plus a `DeploymentDiagnostics` warning, never a crash. Delete it to reset all policy. |
| `AdminNotificationKeyPath` | `admin-notifications.key.json` next to `AdminPasswordPath` | Persisted notification encryption keys (one entry per admin account: fingerprint + random 256-bit key, mode `600`). Same requirements as the password file — writable, and on a persisted volume in a container. Survives restarts by design (an in-memory-only key wiped history on every restart); a password change rotates the entry, which is what clears stored notifications. Unwritable ⇒ in-memory keys ("stable until restart") plus a warning, never a broken endpoint; corrupt ⇒ fresh keys, logged. |
...
| `WebhookAllowPrivateTargets` | `false` | Allow webhook delivery to loopback, link-local and private addresses (RFC1918, CGNAT, IPv6 unique-local), including the cloud metadata endpoint at `169.254.169.254`. Off by default: the endpoints this feature exists for are third-party chat services, while the portal's **Test** button awaits the delivery and reports the upstream status — so an operator-chosen inward URL is an SSRF probe for a network the caller cannot otherwise reach. Enforced at **connect** time on the address actually dialled, so a rebinding DNS answer and a redirect are covered; the marketplace client is unaffected and still allows a loopback `http` mirror. Turn it on for a monitoring agent on the same host or LAN. |
